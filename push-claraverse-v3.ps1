...
Write-Host "  Persistance No Isolation Chat OK V3                           " -ForegroundColor Cyan
...
$branche = "main"
$commitPrefix = "Sauvegarde ClaraVerse V23 - Persistance No Isolation Chat OK V3 - 09 Mars 2026"
...
# Etape 4: Configurer le remote vers le nouveau repository V3
Write-Host ""
Write-Host "4. Configuration du repository distant vers V3..." -ForegroundColor Yellow
$currentRemote = git remote get-url origin 2>&1
if ($currentRemote) {
    Write-Host "  Remote actuel: $currentRemote" -ForegroundColor Gray
    Write-Host "  Changement vers: $repoUrl" -ForegroundColor Yellow
    git remote set-url origin $repoUrl
    Write-Host "  Remote 'origin' mis a jour vers V3" -ForegroundColor Green
} else {
    git remote add origin $repoUrl
    Write-Host "  Remote 'origin' ajoute vers V3" -ForegroundColor Green
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  Attention: Le repository n'existe pas encore ou est vide" -ForegroundColor Yellow
    Write-Host "  Le repository sera cree automatiquement lors du premier push" -ForegroundColor Gray
}
